...
func Pointers() {
	val := 10
	var valPtr *int = &val
	fmt.Println("Val adress: ", valPtr) // Print: 0xc000.....
...
func ArrayPointers() {
	arr := [4]int{1, 2, 3, 4}
	dblArrVals(&arr) // Pass the pointer array to double the values
	fmt.Println(arr) // Print: [2, 4, 6, 8]

	// Passing a slice in an function
	slice := []float64{11, 13, 17}
	fmt.Printf("Average: %.3f\n", average(slice...)) // Print average: 13.667
}
